...
from ...services.models import MainService, ServiceCategory
...
class Command(BaseCommand):
    help = "Seed ServiceCategory and MainService records from the vocational/digital services list."
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            for category_name, service_names in SERVICE_DATA.items():
                ...
                for service_name in service_names:
                    _, svc_created = MainService.objects.get_or_create(
                        name=service_name,
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"\nSeed complete: {created_categories} categories created, "
                f"{created_services} services created, "
                f"{skipped_services} services already existed."
            )
        )
